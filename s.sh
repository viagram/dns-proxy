...
dnsmasq_name=$(curl -skL https://thekelleys.org.uk/dnsmasq/ | egrep -io 'dnsmasq-[0-9]{,2}.[0-9]{,2}(|[0-9a-z]{,4}).tar.gz' | sort -ruV | head -n1)
[[ -f ${dnsmasq_name} ]] && rm -f ${dnsmasq_name}
! wget --no-check-certificate -c "https://thekelleys.org.uk/dnsmasq/${dnsmasq_name}" -O ${dnsmasq_name} && {
    printnew -red "获取失败."
    clear_tmp
    exit 1
}
printnew -green "解压dnsmasq源码"
! tar -zxvf ${dnsmasq_name} && {
    printnew -red "解压失败."
    clear_tmp
    exit 1
}
rm -f ${dnsmasq_name}
cd $(echo ${dnsmasq_name} | sed 's/.tar.gz//g')
...
if [[ x${1} == 'xadd' || x${1} == 'xADD' ]]; then
    ...
    if [[ -n ${iplist} ]]; then
        ...
        echo -e "\033[33m 请输入有效的IP地址\033[0m"
        helper
    fi
